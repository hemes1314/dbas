...
public class TrafficParticiple6 {
	public  List<String> traffic_lines = new ArrayList<String>(); // 分词出来的交通工具
	public  List<String> negative_lines = new ArrayList<String>(); // 分词出来的否定词汇
	public  List<String> traffic_sec_lines = new ArrayList<String>(); // 标准交通工具
	public  List<String> traffic_rest = new ArrayList<String>(); // 存储中间交通工具

	public  HashMap<Integer, String> negative_traffic_map = new HashMap<Integer, String>();// 分词出来的交通工具
	public  HashMap<Integer, String> negative_traffic_sort = new HashMap<Integer, String>();// 分词出来的交通工具
	public  HashMap<Integer, String> traffic_map = new HashMap<Integer, String>();// 分词出来的交通工具
	public  HashMap<Integer, String> negative_map = new HashMap<Integer, String>();// 分词出来的否定词汇
	public  HashMap<String, String> traffic_similty_map = new HashMap<String, String>();// 转换标准交通工具
	...
	public  boolean traffic_con_List(List<String> traffic_list, String targetValue) {
		String[] array = new String[traffic_list.size()];
		traffic_list.toArray(array);
		return Arrays.asList(array).contains(targetValue);
	}

	public  boolean negative_con_List(List<String> negative_list, String targetValue) {
...
//		String traffic = "bdpis-api/src/main/files/traffic.txt"; // 可识别的交通的字典信息
//		String traffic_sec = "bdpis-api/src/main/files/traffic_sec.txt"; // 标准交通的字典信息
//		String negative = "bdpis-api/src/main/files/negative.txt"; // 否定词的字典信息
//		String traffic_similty = "bdpis-api/src/main/files/traffic_similty.txt"; // 近义词转换交通工具的字典信息
		...
		String traffic_similty = this.getClass().getClassLoader().getResource("/").getPath()+"/com/ishansong/files/traffic_similty.txt"; // 近义词转换交通工具的字典信息
		...
		for (Term term : termlist) {
			...
			//}
			// 装载识别出的否定词汇
			//if (term.toString().endsWith("/d") || term.toString().endsWith("/v") || term.toString().endsWith("/vi")) {
				//String[] term_word = term.toString().split("/");
		...
		for (Integer k : negative_map.keySet()) {
			System.out.println(negative_map.get(k));

		}

		for (Integer k : traffic_map.keySet()) {
			System.out.println(traffic_map.get(k));
		...
		if (traffic_map.size() > 0 && negative_map.size() <= 0) { // 只有交通工具，没有否定词
			...
			for (int k : traffic_map.keySet()) {
				traffic_rest.add(traffic_map.get(k));
...
//							}
		...
		traffic_map.clear();
		negative_map.clear();
		negative_traffic_map.clear();
		negative_traffic_sort.clear();
		traffic_similty_map.clear();
		negative_lines.clear();
		traffic_rest.clear();
		traffic_lines.clear();
		traffic_sec_lines.clear();
		traffic_no_str = "";
		traffic_similty_str = "";
...
//		String testline = "不要自行车";
//		TrafficParticiple6 collect = new TrafficParticiple6();
